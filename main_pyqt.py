...
class MainMenuWidget(QtWidgets.QWidget):
    ...
    def _build_ui(self):
        ...
        btn_start.setStyleSheet("background:#3271a8; color:white; font-weight:600; font-size:12pt; border-radius:6px; margin-top:10px;")
        ...
        btn_authors.setStyleSheet("background:#a6b2bd; color:white; font-weight:600; font-size:12pt; border-radius:6px; margin-top:10px;")
        ...
        btn_exit.setStyleSheet("background:#f78765; color:white; font-weight:600; font-size:12pt; border-radius:6px; margin-top:10px;")
...
class SimulationWidget(QtWidgets.QWidget):
    ...
    def _build_ui(self):
        ...
        def add_row(label, widget, unit="", help_text=""):
            ...
            if help_text:
                h = QtWidgets.QLabel(help_text); h.setWordWrap(True); h.setStyleSheet("color:#555;font-size:9pt;margin-top:2px;")
                sp_layout.addWidget(h)
            row.addWidget(lbl)
            row.addWidget(widget)
            if unit:
                u = QtWidgets.QLabel(unit); u.setFixedWidth(80)
                row.addWidget(u)
            sp_layout.addLayout(row)

        self.spin_N = QtWidgets.QSpinBox(); self.spin_N.setRange(5,500); self.spin_N.setValue(100)
        add_row("N", self.spin_N, "шт", "Число частиц")
        self.edit_R = QtWidgets.QDoubleSpinBox(); self.edit_R.setRange(0.003, 0.12); self.edit_R.setSingleStep(0.001); self.edit_R.setValue(0.03)
        add_row("R", self.edit_R, "ед", "Радиус частицы (визуальная единица)")
        self.edit_T = QtWidgets.QDoubleSpinBox(); self.edit_T.setRange(0.01, 5.0); self.edit_T.setSingleStep(0.1); self.edit_T.setValue(0.5)
        add_row("T", self.edit_T, "kT", "Температура (в единицах)")
        self.edit_dt = QtWidgets.QDoubleSpinBox(); self.edit_dt.setRange(0.0001, 0.01); self.edit_dt.setSingleStep(0.0005); self.edit_dt.setValue(0.002)
        add_row("dt", self.edit_dt, "пс", "Шаг интегрирования")

        pot_box = QtWidgets.QComboBox(); pot_box.addItems(["Нет","Отталкивание","Притяжение","Леннард-Джонс","Морзе"])
        self.pot_box = pot_box
        add_row("Pot", pot_box, "", "Тип парного потенциала")
        self.edit_eps = QtWidgets.QDoubleSpinBox(); self.edit_eps.setRange(0.0,50.0); self.edit_eps.setValue(1.0)
        add_row("ε", self.edit_eps, "кДж/моль", "Энергетический масштаб")
        self.edit_sigma = QtWidgets.QDoubleSpinBox(); self.edit_sigma.setRange(0.001, 0.2); self.edit_sigma.setValue(0.02)
        add_row("σ", self.edit_sigma, "ед", "Характерное расстояние LJ")
        self.edit_De = QtWidgets.QDoubleSpinBox(); self.edit_De.setRange(0.0,50.0); self.edit_De.setValue(1.0)
        add_row("De", self.edit_De, "кДж/моль", "Глубина ямы Morse")
        self.edit_a = QtWidgets.QDoubleSpinBox(); self.edit_a.setRange(0.1,50.0); self.edit_a.setValue(2.0)
        add_row("a", self.edit_a, "1/ед", "Жёсткость Morse")
        self.edit_r0 = QtWidgets.QDoubleSpinBox(); self.edit_r0.setRange(0.001,0.5); self.edit_r0.setValue(0.025)
        add_row("r₀", self.edit_r0, "ед", "Равновесное расстояние (Morse)")

        vessel_box = QtWidgets.QComboBox(); vessel_box.addItems(["Прямоугольник","Круг","Многоугольник"])
    ...
    def _init_simulation(self):
        ...
        self.system = System(vessel=vessel, N=100, radius=0.03, temp=0.5, dt=0.002, params=params)
        # build UI first (it needs axes and canvases)
        self._build_ui()
        self.system.seed()

        self.ax_anim.cla()
        self._draw_vessel_patch()
        # sensible scatter size mapping (visual)
        scatter_s = max(2.0, (self.system.radius*1000)**2)
    ...
    def _apply_settings(self):
        ...
        self.system.dt = float(self.edit_dt.value())
        ...
        # vessel mapping from UI text to internal kind
        vk = str(self.vessel_box.currentText())
        if vk in ("rect", "Прямоугольник"):
            self.system.vessel.kind = "Прямоугольник"
            self.system.vessel.rect = (-1,-1,1,1)
            self.system.vessel.circle = (0,0,0)  # Очищаем другие типы
            self.system.vessel.poly = None
        elif vk in ("circle", "Круг"):
            self.system.vessel.kind = "Круг"
            self.system.vessel.circle = (0,0,1)
            self.system.vessel.rect = (0,0,0,0)  # Очищаем другие типы
            self.system.vessel.poly = None
        elif vk in ("poly", "Многоугольник"):
            self.system.vessel.kind = "Многоугольник"
            self.system.vessel.rect = (0,0,0,0)  # Очищаем другие типы
            self.system.vessel.circle = (0,0,0)
            if self.system.vessel.poly is None:
                self.system.vessel.poly = np.array([[-0.8,-0.8],[0.8,-0.8],[0.0,0.8]])
        
        # re-seed system with new parameters
        self.system.seed()

        # Ensure forces history cleared (important)
        if self.system.F is not None:
            self.system.F_old = np.zeros_like(self.system.F)
        else:
            self.system.F_old = None

        # Update or recreate scatter safely
        try:
            if hasattr(self, 'scat') and self.scat is not None:
                # faster path: update offsets instead of removing/creating
                self.scat.set_offsets(self.system.pos)
            else:
                raise AttributeError
        except Exception:
            # fallback: create new scatter
            scatter_s = max(2.0, (self.system.radius*1000)**2)
            self.scat = self.ax_anim.scatter(self.system.pos[:,0], self.system.pos[:,1],
                                             s=scatter_s, c="#215a93", edgecolors='k', linewidths=0.4)

        # redraw vessel patch and adjust view limits to new vessel
        self._draw_vessel_patch()
        self.ax_anim.relim()
    ...
    def _draw_vessel_patch(self):
        ...
        if v.kind in ("rect", "Прямоугольник"):
            ...
            self.ax_anim.set_xlim(xmin-0.1, xmax+0.1); self.ax_anim.set_ylim(ymin-0.1, ymax+0.1)
        elif v.kind in ("circle", "Круг"):
            cx,cy,R = v.circle
            self.vessel_artist = patches.Circle((cx,cy), R, fill=False, lw=2, ec="#444")
            self.ax_anim.set_xlim(cx-R-0.1, cx+R+0.1); self.ax_anim.set_ylim(cy-R-0.1, cy+R+0.1)
            ...
            xmin,ymin = v.poly.min(axis=0); xmax,ymax = v.poly.max(axis=0)
            self.ax_anim.set_xlim(xmin-0.1, xmax+0.1); self.ax_anim.set_ylim(ymin-0.1, ymax+0.1)
        else:
