...
STRINGS = {
    "ru": {
        ...
        "theory.show": "Показать теорию",
        "theory.hide": "Скрыть теорию",
    },
    "en": {
        ...
        "theory.show": "Show theory",
        "theory.hide": "Hide theory",
    },
    "cn": {
...
class TheoreticalDistributions:
    """Класс для вычисления теоретических распределений"""

    @staticmethod
    def normalize_to_histogram(theoretical_y, bin_heights, x_range):
        """Нормализует теоретическую кривую к масштабу гистограммы"""
        if np.max(theoretical_y) <= 0 or np.max(bin_heights) <= 0:
            return theoretical_y

        # Интеграл теоретической кривой
        theory_integral = np.trapz(theoretical_y, x_range)
        if theory_integral <= 0:
            return theoretical_y

        # Интеграл гистограммы (примерно площадь под столбцами)
        hist_integral = np.sum(bin_heights) * (x_range[1] - x_range[0]) / len(bin_heights)
        if hist_integral <= 0:
            return theoretical_y

        # Масштабируем теоретическую кривую
        scale_factor = hist_integral / theory_integral
        return theoretical_y * scale_factor
    ...
    @staticmethod
    def get_theoretical_distribution(vessel_kind, potential_kind, bounds, display_mode=0):
        """
        Возвращает теоретическое распределение для заданных условий

        Args:
            vessel_kind: тип сосуда ('rect', 'circle', 'poly')
            potential_kind: тип потенциала ('Нет', 'Отталкивание', 'Притяжение', 'Леннард-Джонс', 'Морзе')
            bounds: границы области
            display_mode: режим отображения (0 - расстояние, 1 - проекции, 2 - расстояние со знаком)
        """

        # Для проекций и расстояния со знаком - равномерное распределение для прямоугольника
        if display_mode in (1, 2) and vessel_kind == 'rect':
            if display_mode == 1:  # проекции
                x_min, y_min, x_max, y_max = bounds
                width = x_max - x_min
                height = y_max - y_min

                def dist_x(x):
                    return TheoreticalDistributions.uniform_distribution(x, -width / 2, width / 2)

                def dist_y(y):
                    return TheoreticalDistributions.uniform_distribution(y, -height / 2, height / 2)

                def dist_d(d):
                    # Для проекций в прямоугольнике - равномерное распределение
                    max_d = max(width, height)
                    return TheoreticalDistributions.uniform_distribution(d, -max_d, max_d)

                return dist_x, dist_y, dist_d

            elif display_mode == 2:  # расстояние со знаком
                # Для прямоугольника - равномерное распределение
                x_min, y_min, x_max, y_max = bounds
                max_distance = np.sqrt((x_max - x_min) ** 2 + (y_max - y_min) ** 2)

                def dist_d(d):
                    return TheoreticalDistributions.uniform_distribution(d, -max_distance, max_distance)

                return None, None, dist_d

        # Для обычного расстояния (display_mode = 0)
        if vessel_kind == 'rect' and potential_kind in ('Нет', 'None', '无'):
            # Прямоугольник без потенциала - нормальное распределение расстояний
            x_min, y_min, x_max, y_max = bounds
            width = x_max - x_min
            height = y_max - y_min

            # Теоретическое распределение для прямоугольника
            def dist_x(x):
                return TheoreticalDistributions.uniform_distribution(x, x_min, x_max)

            def dist_y(y):
                return TheoreticalDistributions.uniform_distribution(y, y_min, y_max)

            def dist_d(d):
                # Для больших N распределение расстояний стремится к нормальному
                mean_distance = np.sqrt(width ** 2 + height ** 2) / 3  # эмпирическая оценка
                std_distance = mean_distance / 3
                return TheoreticalDistributions.normal_distribution(d, mean_distance, std_distance)

            return dist_x, dist_y, dist_d

        elif vessel_kind == 'circle' and potential_kind in ('Нет', 'None', '无'):
            # Круг без потенциала - распределение Рэлея
            cx, cy, R = bounds
            area = np.pi * R ** 2

            def dist_x(x):
                # Распределение x в круге
                return 2 / (np.pi * R ** 2) * np.sqrt(R ** 2 - x ** 2) if abs(x) <= R else 0

            def dist_y(y):
                # Распределение y в круге (аналогично x)
                return 2 / (np.pi * R ** 2) * np.sqrt(R ** 2 - y ** 2) if abs(y) <= R else 0

            def dist_d(d):
                # Распределение расстояний в круге - Рэлея
                if d <= 0:
                    return 0
                # Масштабирующий параметр для Рэлея
                sigma = R / np.sqrt(2)
                return TheoreticalDistributions.rayleigh_distribution(d, sigma)

            return dist_x, dist_y, dist_d

        elif vessel_kind == 'poly' and potential_kind in ('Нет', 'None', '无'):
            # Многоугольник без потенциала - приближение нормальным распределением
            x_min, y_min, x_max, y_max = bounds
            width = x_max - x_min
            height = y_max - y_min

            def dist_x(x):
                # Приближение равномерным для простоты
                return TheoreticalDistributions.uniform_distribution(x, x_min, x_max)
            ...
            def dist_d(d):
                ...
                mean_distance = np.sqrt(width ** 2 + height ** 2) / 3
                std_distance = mean_distance / 3
                return TheoreticalDistributions.normal_distribution(d, mean_distance, std_distance)

            return dist_x, dist_y, dist_d

        # Для потенциалов отличных от "Нет" - распределение Максвелла-Больцмана
        elif potential_kind not in ('Нет', 'None', '无'):
            x_min, y_min, x_max, y_max = bounds if vessel_kind == 'rect' else (-100, -100, 100, 100)
            width = x_max - x_min
            height = y_max - y_min

            def dist_x(x):
                # При наличии потенциала распределение может отличаться от равномерного
                return TheoreticalDistributions.normal_distribution(x, 0, width / 4)

            def dist_y(y):
                return TheoreticalDistributions.normal_distribution(y, 0, height / 4)

            def dist_d(d):
                # Распределение Максвелла-Больцмана для расстояний при наличии потенциала
                a = np.sqrt(width ** 2 + height ** 2) / 4  # параметр масштаба
                return TheoreticalDistributions.maxwell_boltzmann_distribution(d, a)
        ...
        # По умолчанию - нормальное распределение
            ...
            def dist_x(x):
                return TheoreticalDistributions.normal_distribution(x, 0, 50)

            def dist_y(y):
                return TheoreticalDistributions.normal_distribution(y, 0, 50)

            def dist_d(d):
                return TheoreticalDistributions.normal_distribution(d, 70, 20)
...
class SimulationWidget(QtWidgets.QWidget):
    def __init__(self, parent, back_cb, get_lang_cb):
        ...
        # Флаг отображения теоретических распределений
    ...
    def _build_ui(self):
        ...
        self.edit_T.valueChanged.connect(
            lambda v: (self.edit_T_label.setText(f"{v / 10:.1f}"), self._on_settings_changed()))
        ...
        self.edit_m.valueChanged.connect(
            lambda v: (self.edit_m_label.setText(f"{v / 1000:.2f}"), self._on_settings_changed()))
        ...
        self.edit_eps.valueChanged.connect(
            lambda v: (self.edit_eps_label.setText(f"{v / 10:.1f}"), self._on_settings_changed()))
        ...
        self.edit_sigma.valueChanged.connect(
            lambda v: (self.edit_sigma_label.setText(f"{v / 10:.1f}"), self._on_settings_changed()))
        ...
        self.edit_De.valueChanged.connect(
            lambda v: (self.edit_De_label.setText(f"{v / 10:.1f}"), self._on_settings_changed()))
        ...
        self.edit_a.valueChanged.connect(
            lambda v: (self.edit_a_label.setText(f"{v / 100:.2f}"), self._on_settings_changed()))
        ...
        self.edit_r0.valueChanged.connect(
            lambda v: (self.edit_r0_label.setText(f"{v / 10:.1f}"), self._on_settings_changed()))
        ...
        # Новая кнопка для переключения теоретических распределений
        self.btn_toggle_theory = QtWidgets.QPushButton()
        self.btn_toggle_theory.clicked.connect(self._toggle_theoretical)
        self.btn_toggle_theory.setStyleSheet("""
            QPushButton {
                font-size:13pt; 
                font-weight:600;
                padding: 4px 8px;
            ...
            QPushButton:hover { background: #6d4a8b; }
            QPushButton:pressed { background: #5d3a7b; }
        """)
        ...
        self.btn_toggle_theory.setFixedSize(110, 30)
        top_bar.addWidget(self.btn_toggle_theory)
        ...
        self._update_theory_button()

    def _toggle_theoretical(self):
        """Переключение отображения теоретических распределений"""
        self.show_theoretical = not self.show_theoretical
        self._update_theory_button()
        self._update_histograms()

    def _update_theory_button(self):
        """Обновление текста кнопки теоретических распределений"""
        lang = self.get_lang_cb()
        s = STRINGS[lang.value]

        if self.show_theoretical:
            self.btn_toggle_theory.setText(s.get("theory.hide", "Скрыть теорию"))
        else:
            self.btn_toggle_theory.setText(s.get("theory.show", "Показать теорию"))
    ...
    def _reinitialize_system(self, reset_hist: bool = False):
        ...
        poly = self.system.vessel.poly if self._vessel_kind_to_key(getattr(self.system.vessel, "kind", "")) in (
            "poly", 'Многоугольник') else None
    ...
    def _update_selected_particles_histograms(self):
        ...
        if self.distance_display_mode == 0:  # Обычное расстояние
            ...
            self.ax_histd.set_title(s.get("sim.display_mode.projections", "Проекции расстояния"), fontsize=12,
                                    fontweight='bold')
            ...
            self.ax_histd.set_title(s.get("sim.display_mode.signed", "Расстояние со знаком"), fontsize=12,
                                    fontweight='bold')
    ...
    def _update_histograms(self):
        ...
        # Добавляем теоретические распределения если включено
        if self.show_theoretical:
            self._add_theoretical_distributions()

        self.canvas_hist.draw_idle()

    def _add_theoretical_distributions(self):
        """Добавляет теоретические распределения на графики"""
        if self.system is None or self.system.pos is None:
            return

        # Получаем параметры для теоретических распределений
        vessel_kind = self._vessel_kind_to_key(self.system.vessel.kind)
        potential_kind = self.system.params.kind

        # Получаем границы сосуда
        if vessel_kind == 'rect':
            bounds = self.system.vessel.rect
        elif vessel_kind == 'circle':
            bounds = self.system.vessel.circle
        else:  # poly
            bounds = self.system.vessel.bounds()

        # Получаем теоретические распределения
        dist_x, dist_y, dist_d = TheoreticalDistributions.get_theoretical_distribution(
            vessel_kind, potential_kind, bounds, self.distance_display_mode
        )

        # Добавляем теоретические кривые на графики
        if dist_x is not None and len(self.accumulated_x) > 0:
            x_range = np.linspace(np.min(self.accumulated_x), np.max(self.accumulated_x), 100)
            y_theoretical = dist_x(x_range)

            # Нормализуем к масштабу гистограммы (уменьшаем амплитуду)
            if np.max(y_theoretical) > 0:
                # Получаем данные гистограммы для нормализации
                hist_heights, _ = np.histogram(self.accumulated_x, bins=self.bins_count, density=True)
                if np.max(hist_heights) > 0:
                    # Масштабируем теоретическую кривую чтобы она была НИЖЕ гистограммы
                    scale_factor = 0.3  # Коэффициент для "прижатия вниз"
                    y_theoretical = y_theoretical * (np.max(hist_heights) / np.max(y_theoretical)) * scale_factor

            self.ax_histx.plot(x_range, y_theoretical, 'r-', linewidth=2, alpha=0.8, label='Теория')

        if dist_y is not None and len(self.accumulated_y) > 0:
            y_range = np.linspace(np.min(self.accumulated_y), np.max(self.accumulated_y), 100)
            y_theoretical = dist_y(y_range)

            if np.max(y_theoretical) > 0:
                hist_heights, _ = np.histogram(self.accumulated_y, bins=self.bins_count, density=True)
                if np.max(hist_heights) > 0:
                    scale_factor = 0.3
                    y_theoretical = y_theoretical * (np.max(hist_heights) / np.max(y_theoretical)) * scale_factor

            self.ax_histy.plot(y_range, y_theoretical, 'r-', linewidth=2, alpha=0.8, label='Теория')

        if dist_d is not None:
            if self.distance_display_mode == 0 and len(self.accumulated_distances) > 0:
                d_range = np.linspace(0, np.max(self.accumulated_distances), 100)
                y_theoretical = dist_d(d_range)

                if np.max(y_theoretical) > 0:
                    hist_heights, _ = np.histogram(self.accumulated_distances, bins=self.bins_count, density=True)
                    if np.max(hist_heights) > 0:
                        scale_factor = 0.3
                        y_theoretical = y_theoretical * (np.max(hist_heights) / np.max(y_theoretical)) * scale_factor

                self.ax_histd.plot(d_range, y_theoretical, 'r-', linewidth=2, alpha=0.8, label='Теория')

            elif self.distance_display_mode == 1:
                if len(self.accumulated_dx) > 0:
                    dx_range = np.linspace(np.min(self.accumulated_dx), np.max(self.accumulated_dx), 100)
                    y_theoretical = dist_d(dx_range)

                    if np.max(y_theoretical) > 0:
                        hist_heights, _ = np.histogram(self.accumulated_dx, bins=self.bins_count, density=True)
                        if np.max(hist_heights) > 0:
                            scale_factor = 0.3
                            y_theoretical = y_theoretical * (
                                        np.max(hist_heights) / np.max(y_theoretical)) * scale_factor

                    self.ax_histd.plot(dx_range, y_theoretical, 'r-', linewidth=2, alpha=0.8, label='Теория (Δx)')

                if len(self.accumulated_dy) > 0:
                    dy_range = np.linspace(np.min(self.accumulated_dy), np.max(self.accumulated_dy), 100)
                    y_theoretical = dist_d(dy_range)

                    if np.max(y_theoretical) > 0:
                        hist_heights, _ = np.histogram(self.accumulated_dy, bins=self.bins_count, density=True)
                        if np.max(hist_heights) > 0:
                            scale_factor = 0.3
                            y_theoretical = y_theoretical * (
                                        np.max(hist_heights) / np.max(y_theoretical)) * scale_factor

                    self.ax_histd.plot(dy_range, y_theoretical, 'm-', linewidth=2, alpha=0.8, label='Теория (Δy)')

            elif self.distance_display_mode == 2 and len(self.accumulated_signed) > 0:
                s_range = np.linspace(np.min(self.accumulated_signed), np.max(self.accumulated_signed), 100)
                y_theoretical = dist_d(s_range)

                if np.max(y_theoretical) > 0:
                    hist_heights, _ = np.histogram(self.accumulated_signed, bins=self.bins_count, density=True)
                    if np.max(hist_heights) > 0:
                        scale_factor = 0.3
                        y_theoretical = y_theoretical * (np.max(hist_heights) / np.max(y_theoretical)) * scale_factor

                self.ax_histd.plot(s_range, y_theoretical, 'r-', linewidth=2, alpha=0.8, label='Теория')

        # Добавляем легенды
        if dist_x is not None and len(self.accumulated_x) > 0:
            self.ax_histx.legend()
        if dist_y is not None and len(self.accumulated_y) > 0:
            self.ax_histy.legend()
        if dist_d is not None:
            self.ax_histd.legend()
    ...
    def _update_all_particles_histograms(self):
        ...
        if self.distance_display_mode == 0:  # Обычное расстояние
            ...
            self.ax_histd.set_title(s.get("sim.display_mode.projections", "Проекции расстояния"), fontsize=10,
                                    fontweight='bold')
            ...
            self.ax_histd.set_title(s.get("sim.display_mode.signed", "Расстояние со знаком"), fontsize=10,
                                    fontweight='bold')
            # Добавляем вертикальную линию в нуле
            self.ax_histd.axvline(x=0, color='red', linestyle='--', alpha=0.7)
    ...
    def update_language(self, lang: Lang):
        ...
        # Обновляем состояние кнопок полигона после сменя языка
        self._update_polygon_buttons_state()
...
class SimulationWidget(QtWidgets.QWidget):
    ...
    def _add_theoretical_distributions(self):
        ...
        if dist_x is not None and len(self.accumulated_x) > 0:
            x_range = np.linspace(np.min(self.accumulated_x), np.max(self.accumulated_x), 100)
            y_theoretical = dist_x(x_range)
            # Нормализуем чтобы площадь под кривой была примерно 1
            if np.max(y_theoretical) > 0:
                y_theoretical = y_theoretical / np.trapz(y_theoretical, x_range) * np.ptp(x_range) / 10
            self.ax_histx.plot(x_range, y_theoretical, 'r-', linewidth=2, alpha=0.8, label='Теория')
        ...
        if dist_y is not None and len(self.accumulated_y) > 0:
            ...
            y_theoretical = dist_y(y_range)
            if np.max(y_theoretical) > 0:
                y_theoretical = y_theoretical / np.trapz(y_theoretical, y_range) * np.ptp(y_range) / 10
            self.ax_histy.plot(y_range, y_theoretical, 'r-', linewidth=2, alpha=0.8, label='Теория')
        ...
        if dist_d is not None:
            if self.distance_display_mode == 0 and len(self.accumulated_distances) > 0:
                ...
                y_theoretical = dist_d(d_range)
                if np.max(y_theoretical) > 0:
                    y_theoretical = y_theoretical / np.trapz(y_theoretical, d_range) * np.ptp(d_range) / 10
                self.ax_histd.plot(d_range, y_theoretical, 'r-', linewidth=2, alpha=0.8, label='Теория')
                ...
                if len(self.accumulated_dx) > 0:
                    ...
                    y_theoretical = dist_d(dx_range)
                    if np.max(y_theoretical) > 0:
                        y_theoretical = y_theoretical / np.trapz(y_theoretical, dx_range) * np.ptp(dx_range) / 10
                    self.ax_histd.plot(dx_range, y_theoretical, 'r-', linewidth=2, alpha=0.8, label='Теория (Δx)')
                ...
                if len(self.accumulated_dy) > 0:
                    ...
                    y_theoretical = dist_d(dy_range)
                    if np.max(y_theoretical) > 0:
                        y_theoretical = y_theoretical / np.trapz(y_theoretical, dy_range) * np.ptp(dy_range) / 10
                    self.ax_histd.plot(dy_range, y_theoretical, 'm-', linewidth=2, alpha=0.8, label='Теория (Δy)')
                ...
                y_theoretical = dist_d(s_range)
                if np.max(y_theoretical) > 0:
                    y_theoretical = y_theoretical / np.trapz(y_theoretical, s_range) * np.ptp(s_range) / 10
                self.ax_histd.plot(s_range, y_theoretical, 'r-', linewidth=2, alpha=0.8, label='Теория')
